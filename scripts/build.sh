...
function publishpackage()
{
    ...
    fullDestDir=$coreCLRPackageDir/$netFull_Dir
    mkdir -p $fullDestDir
    if [[ $? -ne 0 ]]; then
        echo "Invoke Build failed."
        exit
...
function createnugetpackages()
{
    ...
    for i in ${projectFiles[@]}; do
    	echo "$DOTNET_PATH pack --no-build $stagingDir/${i} -o $binDir --version-suffix $TPB_VersionSuffix" /p:PackageVersion=$TPB_Version \
    	&& $DOTNET_PATH pack --no-build $stagingDir/${i} -o $binDir --version-suffix $TPB_VersionSuffix /p:PackageVersion=$TPB_Version
    done
